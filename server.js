...
app.put('/api/medications/:id', async (req, res) => {
  try {
    ...
    res.json({ message: 'Medication updated successfully', updatedMed });
  } catch (err) {
...
app.post('/api/medications/add', async (req, res) => {
  try {
    const { batchNumber, name, availableQuantity, soldQuantity } = req.body;
    const newMed = new MedicationStock({ batchNumber, name, availableQuantity, soldQuantity });
    await newMed.save();
    res.json({ message: 'Medication added successfully', newMed });
  } catch (err) {
...
app.get('/api/medications', async (req, res) => {
  try {
    const meds = await MedicationStock.find();
    res.json(meds);
  } catch (err) {
...
app.get('/api/medications/low-stock', async (req, res) => {
  try {
    const lowStockMeds = await MedicationStock.find({ availableQuantity: { $lt: 5 } });
